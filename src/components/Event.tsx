  ...
  return (
    <div
      ...
      className={`group relative p-6 md:p-8 border-2 transition-all duration-300 cursor-pointer ${rotationClass} hover:rotate-0 hover:-translate-y-2 hover:shadow-[8px_8px_0px_0px_rgba(204,255,0,1)] ${
        isDark
          ? "bg-[#0a0a0a] text-white border-zinc-800 hover:border-[#ccff00]"
    ...
    >
      <div className={`flex items-center justify-between border-b pb-4 mb-6 font-mono text-xs ${isDark ? "border-zinc-800 text-zinc-400" : "border-black/20 text-black/70"}`}>
        <span className="font-bold tracking-widest text-[#ccff00] bg-black px-2 py-0.5 border border-zinc-700">
          {"// " + props.type.toUpperCase()}
        </span>
        <span className="flex items-center gap-1 font-bold">
          <IoCalendarOutline className="w-3.5 h-3.5 text-[#ccff00]" />
          {props.date}
        </span>
      </div>
      ...
      <div className="overflow-hidden border border-black mb-6 relative bg-zinc-900">
        ...
        />
        <div className="absolute top-2 left-2 bg-black/90 text-[#ccff00] font-mono text-[10px] px-2 py-0.5 border border-zinc-700">
          EVENT_IMG.{cardIndex + 1}
        </div>
        <div className="absolute bottom-2 right-2 text-[#ccff00] font-mono text-xs opacity-0 group-hover:opacity-100 transition-opacity">
          EXPAND ↗
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        <h3 className={`text-3xl sm:text-4xl font-black font-oswald uppercase leading-none tracking-tight transition-colors ${isDark ? "group-hover:text-[#ccff00]" : "group-hover:text-black underline decoration-[#ccff00] decoration-4"}`}>
          {props.name}
        </h3>

        <div className={`flex items-center gap-2 font-mono text-xs pt-2 ${isDark ? "text-zinc-400" : "text-zinc-700"}`}>
          <RiMapPinLine className="w-4 h-4 text-[#ccff00]" />
      ...
      <div className={`mt-6 pt-4 border-t flex items-center justify-between font-mono text-xs ${isDark ? "border-zinc-800" : "border-black/20"}`}>
        <button
          ...
          className="bg-[#ccff00] text-black font-bold px-4 py-2 hover:bg-white transition-colors border border-black flex items-center gap-1 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
        >
          <span>GET SPOT</span>
          <span>↗</span>
        </button>

        <span className="group-hover:translate-x-1 transition-transform text-[#ccff00] font-bold">
          DETAILS ↗
        </span>
      </div>
    </div>
