...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Pointer-hover interaction (used whenever voice control is off/unavailable)
      let pointerActive = false;
      let hoverLevel = 0;
      const onPointerEnter = () => { pointerActive = true; };
      ...
      const update = (t: number) => {
        ...
        if (enableVoiceControl && isMicrophoneInitialized) {
          ...
          // No voice input: drive the same hover/rotation effects from pointer hover instead
          const hoverTarget = pointerActive ? 1 : 0;
          hoverLevel += (hoverTarget - hoverLevel) * Math.min(dt * 4, 1);

          const hoverRotationSpeed = baseRotationSpeed + hoverLevel * maxRotationSpeed;
          currentRot += dt * hoverRotationSpeed;

          program.uniforms.hover.value = hoverLevel;
          program.uniforms.hoverIntensity.value = hoverLevel * maxHoverIntensity;
