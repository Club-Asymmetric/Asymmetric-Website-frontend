...
const FALLBACK_EVENTS: EventData[] = [
  {
    ...
    description: "A high-octane night of creative hacking, audio-visual experiments, live demos, and unexpected collaborations. Join us as campus transforms into a playground of sound, light, and code.",
    date: "2025-10-12",
    ...
    max_team_size: 4,
    participants: 120,
    photos: ["/group-photo/CatapultProgram3.jpg"],
    category: "CULTURE"
  },
  {
    id: "evt-2",
    name: "BUILD-A-THON 3.0",
    synopsis: "24 Hours of Unfiltered Creation",
    description: "No pitch decks, no corporate sponsor fluff. Just 24 hours to turn your wildest technical ideas into working software or hardware prototypes. Raw, fast, and competitive.",
    date: "2025-11-05",
    registration_start: "2025-10-01",
    location: "Innovation Lab 04",
    min_team_size: 2,
    max_team_size: 4,
    participants: 80,
    photos: ["/group-photo/CatapultProgram.jpg"],
    category: "HACKATHON"
  },
  {
    id: "evt-3",
    name: "SYSTEMS & SYNTAX WORKSHOP",
    synopsis: "Deep Dive into Low-Level Magic",
    description: "Deconstruct how modern operating systems, compilers, and memory management actually work. Hands-on terminal sessions for students who want to master underlying architectures.",
    date: "2025-11-20",
    ...
    max_team_size: 1,
    participants: 50,
    photos: ["/group-photo/FirstMeet.jpg"],
    category: "WORKSHOP"
  },
  {
    id: "evt-4",
    name: "UNDERGROUND HACK NIGHT",
    synopsis: "Late Night Code & Pizza",
    description: "An open venue session where member teams bring ongoing projects, debug together, exchange code reviews, and experiment with novel frameworks in a relaxed atmosphere.",
    date: "2025-12-01",
    registration_start: "2025-11-10",
    location: "Student Center Lounge",
    min_team_size: 1,
    max_team_size: 3,
    participants: 60,
    photos: ["/group-photo/JuniorMeet.jpg"],
    category: "TECH"
  },
  {
    id: "evt-5",
    name: "AI & ART COLLISION",
    synopsis: "Generative Models meet Visual Culture",
    description: "Exploring creative coding, generative neural networks, and interactive digital art. Learn how to train custom models and craft responsive generative installations.",
    date: "2025-12-15",
    registration_start: "2025-11-25",
    location: "Design Studio 01",
    min_team_size: 1,
    max_team_size: 2,
    participants: 45,
    photos: ["/group-photo/ShootingVideo.jpg"],
    category: "TALK"
  }
];
    ...
    const fetchEvents = async () => {
      try {
        ...
        if (response.status === 200 && response.data) {
          const eventsArray = Object.values(response.data) as EventData[];
          setEvents(eventsArray.length > 0 ? eventsArray : FALLBACK_EVENTS);
        } else {
          setEvents(FALLBACK_EVENTS);
        }
      } catch (error) {
        console.warn("Backend API unreachable, using curated events fallback:", error);
        setEvents(FALLBACK_EVENTS);
      } finally {
        setLoading(false);
      }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10 items-start">
            {filteredEvents.map((event, idx) => (
              <Event
                ...
                index={idx}
                imageSrc={
                  event.photos && event.photos[0] && event.photos[0].startsWith("/")
                    ? event.photos[0]
                    : event.photos && event.photos[0] && localhost
                    ? `${localhost}/images/are/not/here/${event.photos[0]}`
